  ...
  return (
    <div className="min-h-screen">
      <header className="text-center pt-28 pb-8 px-4">
        ...
        <p className="text-zinc-500 text-sm mt-2">Viral content templates · One-click copy</p>
      </header>
        ...
        <div className="flex gap-1.5 mb-6 overflow-x-auto pb-2">
          <button onClick={() => setActiveCategory('all')}
            className={cn('px-3 py-1.5 rounded-full text-[10px] font-semibold shrink-0 transition',
              activeCategory === 'all' ? 'bg-violet-500/15 text-violet-400 border border-violet-500/20' : 'text-zinc-600 hover:text-zinc-400')}>All</button>
          {categories.map(cat => (
            ...
            <div key={tpl.id} className="glass-elevated overflow-hidden">
              ...
              {expanded === tpl.id && (
                <div className="px-5 pb-5 border-t border-white/[0.03] pt-4 animate-in">
                  <div className="mb-3">
                    <span className="section-label mb-1.5 block">Title</span>
                  ...
                  <div className="mb-3">
                    <span className="section-label mb-1.5 block">Content</span>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => handleCopy(tpl)} className={cn('btn-ghost px-4 py-2 text-xs flex items-center gap-1.5', copied === tpl.id && '!text-green-400 !border-green-500/20')}>
                      {copied === tpl.id ? <><Check size={12} /> Copied</> : <><Copy size={12} /> Copy</>}
                    </button>
                    <a href={`/?template=${tpl.id}`} className="btn-ghost px-4 py-2 text-xs">Check →</a>
                  </div>
                </div>
              )}
            </div>
