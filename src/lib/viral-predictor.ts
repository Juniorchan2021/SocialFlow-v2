...
export function predictViral(
  ...
  // Pattern matching uses CONTENT language, not UI language
  const patterns = lang === 'zh' ? VIRAL_PATTERNS_ZH : VIRAL_PATTERNS_EN;
  const formulaMatches = patterns.filter(p => p.test(full)).length;
  const formulaScore = Math.min(100, formulaMatches * 20);
  factors.push({
    name: t('爆文公式匹配', 'Viral formula match'),
    score: formulaScore, weight: 0.3,
    detail: t(`匹配了${formulaMatches}个爆文模式`, `Matched ${formulaMatches} viral patterns`),
  });
  ...
  factors.push({
    name: t('情绪张力', 'Emotional tension'),
    score: emotionScore, weight: 0.25,
    detail: t(`情绪词密度${emotionScore >= 50 ? '充足' : '偏低'}`, `Emotional word density is ${emotionScore >= 50 ? 'strong' : 'weak'}`),
  });
  ...
  factors.push({
    name: t('内容类型适配', 'Content type fit'),
    score: typeScore, weight: 0.2,
    detail: t(`${contentType}类型在${platform}上的传播力`, `${contentType} virality on ${platform}`),
  });
  ...
  if (formulaScore < 60) {
    boostTips.push(t('套用爆文公式："[数字]+[动作]+[惊喜结果]"', 'Apply viral formula: "[Number] + [Action] + [Surprising Result]"'));
  }
  if (emotionScore < 50) {
    boostTips.push(t('增加情绪词（震惊/绝了/救命）制造情绪张力', 'Add emotion words (amazing/shocking/unbelievable) for tension'));
  }
  if (hookScore < 60) {
    boostTips.push(t('标题加数字+钩子词，点击率提升36-80%', 'Add number + hook to title for 36-80% CTR boost'));
  }
  if (!structure.hasQuestion) {
    boostTips.push(t('加提问句激发讨论，评论互动是传播关键', 'Add a question to spark discussion — comments drive shares'));
  }
