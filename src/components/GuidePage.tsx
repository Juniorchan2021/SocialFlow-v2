...
interface Props { icon: string; title: string; description: string; sections: Section[]; relatedLinks: { label: string; href: string }[]; }
  ...
  return (
    <div className="min-h-screen">
      <header className="max-w-3xl mx-auto px-4 pt-28 pb-10">
        ...
        <h1 className="text-3xl font-extrabold tracking-tight text-white mb-3">{title}</h1>
      ...
      <main className="max-w-3xl mx-auto px-4 pb-20">
        ...
        <div className="mt-12 glass-elevated p-8 text-center">
          <p className="text-sm text-zinc-400 mb-4">用 SocialFlow 检测你的内容是否符合以上规则</p>
          <a href="/" className="btn-primary inline-flex items-center gap-2 px-8 py-3 text-sm"><Sparkles size={14} /> Free Check</a>
        </div>

        {relatedLinks.length > 0 && (
          <div className="mt-8">
            <span className="section-label mb-3 block">Related</span>
            <div className="flex flex-wrap gap-2">{relatedLinks.map((link, i) => (
              <a key={i} href={link.href} className="btn-ghost px-4 py-2 text-xs">{link.label}</a>
            ))}</div>
          </div>
        )}
      </main>
